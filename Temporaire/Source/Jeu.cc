...
void Jeu::ajouter_Info(Gendarme const &G)
{
	Information_Gendarme Info(G);
	Info_Gendarmes.push_back(Info.clone());
}
...
void Jeu::Afficher(){
	...
	std::cout<<"Liste Information_Gendarme :\n";
	for (auto i : getInfo())
	{
		std::cout<<i->getGendarme().getNom()<<" position :"<<i->getGendarme().Affiche_Position()<<"\n";
	}
}
